...
UwbReceiver& UwbReceiver::instance() {
    static UwbReceiver inst;
    return inst;
}

bool UwbReceiver::start(const std::string& config_path) {
    Config::instance().load(config_path);
    const auto unitree_cfg = Config::instance().root()["unitree"];
    return start(unitree_cfg["domain_id"].as<int>(),
                 unitree_cfg["network_interface"].as<std::string>());
}
...
bool UwbReceiver::start(int domain_id, const std::string& network_interface,
    ...
    try {
        ...
        std::cerr << "[UwbReceiver] DDS init failed on interface \""
                  << network_interface << "\": " << e.what() << "\n";
        return false;
    }

    stop_watchdog_ = false;
    watchdog_thread_ = std::thread(&UwbReceiver::watchdog_loop, this);
    std::cout << "[UwbReceiver] started on domain=" << domain_id
...
void UwbReceiver::set_on_position(OnPositionFn fn) {
    on_position_ = std::move(fn);
}

void UwbReceiver::on_pose_update(const void* message) {
...
void UwbReceiver::watchdog_loop() {
    ...
    while (!stop_watchdog_) {
        ...
        if (fix.HasData() && fix.GetAgeMs() > stale_ms) {
            std::cerr << "[UwbReceiver] UWB fixes stale — cleared\n";
            uwb_buf.Clear();
        }
    }
}
